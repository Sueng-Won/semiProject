...
public class MyResumeService {
	...
	public ArrayList<MyResumeVo> selectResumeCondition(int currentPage, int limit, String searchJob,
		...
		ArrayList<MyResumeVo> jsList = new MyResumeDao().selectResumeCondition(con, currentPage, limit, searchJob, searchLocal, searchworkTime);
	...
	public int selectJSTotalCount(String searchJob, String searchLocal , String searchworkTime) {
		//페이지.. 조건에 맞는 결과 가져오기..
				//1. 커넥션 연결
						Connection con = JDBCTemplate.getConnection();
						//2. dao 메소드 호출
						int listCount = new MyResumeDao().selectJSTotalCount(con, searchJob, searchLocal, searchworkTime);
						//3. 자원 반납(close)
						JDBCTemplate.close(con);
						//4. 해당 결과 리턴
						return listCount;

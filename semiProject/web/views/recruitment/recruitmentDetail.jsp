		...
		<div class="col-lg-9 mt-lg-auto">
			<div class="row mt-4" style="margin-left: 50px;">
				<div class="title_line">
					<div><h5><%=rec.getRecruitment_name() %></h5></div>
					<div>
				...
				<div class="line">
					<div class="title_image">
						<div>
							<img src="<%if(rec.getRecruitment_image_src()==null){ %>/sp/images/building.jpeg<%}else{%>/sp/images/recruitmentImg/<%=rec.getRecruitment_image_src()%><%}%>"
				...
				<div class="line">
					<div style="float: left;">
						...
						<table>
							...
							<tr>
								<td>희망성별</td>
								<td><%if(rec.getGender()=='M'){ %>남성우대<%}else if(rec.getGender()=='F'){ %>여성우대<%}else{ %>무관<%} %></td>
							</tr>
							<tr>
								<td>희망경력</td>
								<td><%if(rec.getCareer()==0){ %>무관<%}else{ %>필요<%} %></td>
							</tr>
							<tr>
								<td>희망학력</td>
							...
							<tr>
								<td>병역우대</td>
								<td><%if(rec.getMilitary_service()==0){ %>무관<%}else{ %>군필우대<%} %></td>
							</tr>
						</table>
					</div>
					<div style="float: right; margin-right: 50px;">
						...
						<table>
							<tr>
								<td>채용자</td>
								<td><%=writer.getName().charAt(0) %><%for(int i=0;i<writer.getName().length()-3;i++){%>*<%}%><%=writer.getName().charAt(writer.getName().length()-1) %></td>
							</tr>
							<tr>
								<td>연락처</td>
								<td><%if(id==null){ %>로그인후 확인이 가능합니다.<%}else{ %><%=rec.getRecruitment_phone()%><%} %></td>
							</tr>
						</table>
					</div> 
				</div>

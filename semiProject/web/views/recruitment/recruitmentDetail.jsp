					...
					<div style="float: left;">
						...
						<table>
							...
							<tr>
								<td>희망성별</td>
								<td>
									<%if(rec.getGender()=='M'){ %>남성우대<%}else if(rec.getGender()=='F'){ %>여성우대<%}else{ %>무관<%} %>
								</td>
							</tr>
							<tr>
								<td>희망경력</td>
								<td>
									<%if(rec.getCareer()==0){ %>무관<%}else{ %>필요<%} %>
								</td>
							</tr>
							<tr>
								<td>희망학력</td>
							...
							<tr>
								<td>병역우대</td>
								<td>
									<%if(rec.getMilitary_service()==0){ %>무관<%}else{ %>군필우대<%} %>
								</td>
							</tr>
						</table>
					</div>
						...
						<table>
							<tr>
								...
								<td><%=writer.getName().charAt(0) %>
									<%for(int i=0;i<writer.getName().length()-3;i++){%>*<%}%><%=writer.getName().charAt(writer.getName().length()-1) %></td>
								<%}else{ %>
							...
							<tr>
								<td>연락처</td>
								<td>
									<%if(id==null){ %>로그인후 확인이 가능합니다.<%}else{ %><%=rec.getRecruitment_phone()%>
									<%} %>
								</td>
							</tr>
						</table>
